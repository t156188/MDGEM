...
- **AI** (`AIPanel`): a streaming **agent** over an OpenAI-compatible provider (presets in `settings.entry.js#AI_PRESETS`: 智谱 GLM, 智谱 GLM Coding Plan, DeepSeek, 通义千问, 自定义 — all hit `{baseURL}/chat/completions`). HTTP runs **natively** (`AIService.swift` URLSession streaming / `lib.rs ai_chat_stream` reqwest blocking + SSE), never page `fetch`, to dodge `file://` CORS and keep the key out of the web context. Config persists in UserDefaults (mac) / tauri-store key `aiConfig` (win).
  ...
  - **History** (`AIPanel`): multi-session, per-workspace. `convo` is snapshotted into a `sessions` list after every agent turn and on ✚-new-chat; the 🕘 overlay lists/loads/deletes (**soft delete** — flags the record `deleted:true` and keeps the file, no recovery UI) past conversations. Persisted via the `historyLoad/historySave` IPC with `scope:'ai'`, keyed by workspace root path.
- **Terminal** (`TerminalPanel`): real interactive PTY via **hterm** (Chromium libapps; prebuilt IIFE `mac/build-web/vendor-src/hterm_all.js` → copied to `Resources/vendor/hterm_all.js`, exposing `window.MDTerm = { lib, hterm }`, lazy-loaded on first panel open). **hterm replaced xterm.js** because xterm's keydown/IME path under WKWebView (mac) doubled chars, needed two presses for shifted symbols, and added stray spaces on CJK candidate selection; hterm's textarea/composition input handles IME correctly. The swap is **front-end only** — the IPC byte-stream contract is renderer-agnostic, so `PTYSession.swift`/`portable-pty` are untouched. Each terminal is `new hterm.Terminal({storage: lib.Storage.Memory})` decorated into a `.term-host` (hterm renders inside its own iframe); in `onTerminalReady` we push an io and wire `io.onVTKeystroke`/`io.sendString` → `handleTermInput` (same resolved-string funnel xterm's `onData` gave us, so the ghost/command-tracking logic is reused verbatim), `io.onTerminalResize` → `requestTermResize`, a `Shift+ENTER`→`ESC+CR` keyboard binding, and spawn the pty with `term.screenSize`. Output is `io.print(data)`; theme/font go through `term.getPrefs().set(...)` (`applyHtermPrefs`/`applyFontSize`). Backends: `PTYSession.swift` (posix_openpt + Process) on mac, `portable-pty` (`TermState` in `lib.rs`) on win. Bytes flow as utf8 strings. IPC: `termCreate/termInput/termResize/termKill` ↔ `onTermData(id,data)/onTermExit(id,code)`, keyed by a JS-generated terminal id. The panel is **multi-window**: a left rail (`.term-tabs`) lists sessions (new `+` / rename on double-click / close `×`); each is its own PTY id. hterm has no SerializeAddon, so replay-on-reopen rebuilds **colored ANSI** from the rendered row DOM (`serializeSession` → `rowToAnsi`): hterm styles each span inline (`style.color`/`backgroundColor` as `rgb(var(--hterm-color-N))` for the palette or literal `rgb()` for truecolor; `fontWeight`/`fontStyle`/`textDecoration`/`.faint`), so each span becomes an SGR run — palette entries as 256-color `38;5;N` (the replay terminal has the same palette, so they render identically), truecolor as 24-bit. Rows are joined into **logical lines** (wrapped rows carry a `line-overflow` attr → no `\n`), so the stored text re-wraps naturally at whatever width the terminal has on reopen instead of a raw byte stream re-wrapping wrong. Then (testing an SGR-stripped copy of each line) it drops everything up to the **last** `DIVIDER_MARK` line (so a prior reopen's history + old dividers are never re-saved — no compounding, replacing xterm's `registerMarker` boundary), filters zsh's partial-line `%` (PROMPT_SP) marker lines, strips trailing blanks, and caps to the last 600 lines. On restore the blob is printed as-is (`\n`→`\r\n`), then `SESSION_DIVIDER`, then the fresh shell. `snapshotSessions` **skips a terminal while a full-screen app owns the screen** (alt-buffer/mouse modes in `track.modes` — keeps `dirty`, defers) so a running `claude`/`vim`/`less` TUI isn't captured as history. Snapshotted to **its own file** `term/<pid>.json` (throttled; only terminals dirtied since the last snapshot are rewritten). Theme colors + the ANSI-16 palette (a curated dark set / light set chosen by the theme bg's luminance) are pushed via `applyHtermPrefs` on create and every theme switch. Closing a window is a **soft delete** — its file is rewritten flagged `deleted:true` (last output kept, filtered out by `restoreOrCreate`, not replayed). The fish-style ghost overlay lives **inside the hterm iframe** (`installGhost` appends to the scrollport document), positioned at `term.cursorNode_`'s offsets; refreshed via a per-terminal rAF (`scheduleGhost`) since hterm has no `onRender`.
...
```
MDGEM/chat/index.json   [{id,workspace,title,createdAt,updatedAt,deleted?}, …]
MDGEM/chat/<id>.json    {id,workspace,title,createdAt,updatedAt,deleted?, messages:[…]}   one file per conversation
MDGEM/term/commands.json {id:"commands", commands:{ "<workspace>": [{cmd,count,lastUsed,deleted?}, …] }}  single record
MDGEM/term/index.json    [{id,workspace,title,createdAt,updatedAt,deleted?}, …]   (terminal per-window records + the fixed commands record)
MDGEM/term/<pid>.json    {id,workspace,name,createdAt,updatedAt,deleted?, data:"<serialized ANSI blob>"}   one file per terminal window
```
...
- **term** = the fixed-id `"commands"` record **plus one file per terminal window** (the latter mirror chat's one-file-per-conversation layout, sharing the same `index.json`). `term/commands.json` is a `{ "<workspace>": [{cmd,count,lastUsed,deleted?}] }` frequent-commands table — the terminal learns commands as you type (`recordCommand` bumps count + recency; re-typing a soft-deleted command revives it) and drives the inline ghost suggestion from the current workspace's slice; **management (delete a command / 清空全部) lives in the Settings window → 终端**, which merges all workspaces into one 常用 list. `term/<pid>.json` files are the per-window terminal output snapshots (one captured raw-output blob each) for replay-on-reopen (see the Terminal bullet above); `restoreOrCreate` reads `term/index.json`, filters to this workspace's non-deleted records (skipping the `commands` record and any stale single `sessions` record from the old single-file layout — **not migrated**, left ignored), then reads each `<pid>.json`. Deleting a command, 清空全部, and closing a terminal window are all **soft deletes** (flag `deleted:true`, keep the data, no recovery UI; filtered out at read/suggestion time). There is **no** live terminal transcript beyond these snapshots.

Four IPC verbs replace the old load/save pair, all resolving on the existing `onHistoryLoaded(reqId,value)` callback: `historyList(scope)` → index array, `historyRead(scope,id)` → record|null, `historyWrite(scope,id,record)` → `{ok:true}`, `historyDelete(scope,id)` → `{ok:true}`. Mac script handlers (`historyList/Read/Write/Delete` in `MarkdownWebView.swift`) → `HistoryStore.swift`; win `mdreader:history-{list,read,write,delete}` listeners in `lib.rs`; the `mdreader:history-loaded` → `onHistoryLoaded` bridge translation is unchanged (all four verbs reply on it). **Native maintains `index.json`** by lifting a fixed `META_KEYS`/`metaKeys` allow-list (`id,workspace,title,createdAt,updatedAt,deleted`) from each written record — the front-end never writes the index directly. (`deleted` is in the allow-list so chat soft deletes — a record re-written with `deleted:true` — propagate to the index and `loadHistory` can filter them out.) `id` is **sanitized** to `[A-Za-z0-9_]` natively (it's used as a filename) to prevent path traversal. Old `history-ai.json`/`history-term.json` from before this layout are not migrated — left in place, ignored.
...
- `Resources/` (with `type: folder`) is copied wholesale into the bundle. `Resources/vendor/` is produced by `mac/build-web/build.mjs` (esbuild) and **gitignored** (`mac/.gitignore`); the `mac/build-web` step runs as part of `make web` / `make build`. build.mjs emits IIFE bundles — `viewer.bundle.js`, `mermaid.bundle.js`, `editor.bundle.js`, `settings.bundle.js` — plus the hljs/KaTeX CSS copies, and **copies** the vendored hterm bundle `vendor-src/hterm_all.js` → `vendor/hterm_all.js` (the terminal engine; `window.MDTerm = { lib, hterm }`, lazy-loaded on first terminal open). `hterm_all.js` is **not** built by esbuild — it's a prebuilt Chromium-libapps artifact committed under `vendor-src/`; regenerate per `vendor-src/README.md` (clone libapps, rollup libdot+hterm, esbuild a `{lib,hterm}` re-export to an IIFE). The old xterm.js deps and `terminal.entry.js` are gone.
